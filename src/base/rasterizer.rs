...
pub fn draw_triangle(context: &mut Context, triangle: &Triangle, transform: Matrix4<f32>) {
    ...
    for y in miny..maxy { // For Y in bounds
        for x in minx..maxx { // For X in bounds
            ...
            if w0 >= 0.0 && w1 >= 0.0 && w2 >=0.0 {                   // Backface culling isn't necessary because it does not render the backfaces anyway
                let pixel_shade = shade*(w0 + w1 + w2);               // This is the lighting component
                let z = &dist_triangle.v1[2] + a*(w1*(&dist_triangle.v2[2] - &dist_triangle.v1[2]) + w2*(&dist_triangle.v3[2] - &dist_triangle.v1[2])); // Z-test
